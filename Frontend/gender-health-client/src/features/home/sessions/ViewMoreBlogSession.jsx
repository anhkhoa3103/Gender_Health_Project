...
const ViewMoreBlogSession = () => {
    ...
    return (
        <div className="page-wrapper_bloghome">
            <div className="header-wrapper_bloghome">
                <Header />
            </div>

            <section className="view-more-container_bloghome">
                <h1 className="section-title_bloghome">Our Blogs</h1>
                <div className="section-subtitle_bloghome">HEALTH & NEWS CORNER</div>

                <div className="blog-grid_bloghome">
                    {visibleBlogs.map((blog) => (
                        <div key={blog.id} className="blog-card_bloghome">
                            <img src={blog.img} alt={blog.title} className="blog-image_bloghome" />
                            <div className="blog-content_bloghome">
                                <h2 className="blog-title_bloghome">{blog.title}</h2>
                                <p className="blog-meta_bloghome">{blog.date} · {blog.readTime}</p>
                                <p className="blog-description_bloghome">{blog.desc}</p>
                                <Link to={`/blog/${blog.id}`} className="view-button_bloghome">
                                    View details
                                </Link>
                            </div>
                        </div>
